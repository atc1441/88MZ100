...
static bool prvApplyUpdateIfNeeded(struct Settings *settings) // return true if a reboot is needed
{
    ...
    FLASH_Read(0, EEPROM_UPDATE_START, (uint8_t *)&eih, sizeof(struct EepromImageHeader));
    ...
    if (eih.version > mCurVersionExport)
    {
        ...
        for (ofst = 0; ofst < size; ofst += now)
        {
            ...
            FLASH_Read(0, EEPROM_UPDATE_START + ofst + sizeof(struct EepromImageHeader), chunkStore, now);
...
static uint32_t prvDriveDownload(struct Settings *settings, struct CommsInfo *ci, struct EepromImageHeader *eih, uint32_t addr, bool isOS)
{
    struct
    {
        uint8_t pktTyp;
    ...
    for (; curPiece < nPieces; curPiece++)
    {
        ...
        for (nRetries = 0; nRetries < 5; nRetries++)
        {
            ...
            while (1)
            {
                ...
                // write data
                FLASH_Write(false, addr + curPiece * EEPROM_PIECE_SZ + sizeof(struct EepromImageHeader), data, ret);

                // write marker
                eih->piecesMissing[curPiece / 8] &= ~(1 << (curPiece % 8));
                FLASH_Write(false, addr + offsetof(struct EepromImageHeader, piecesMissing[curPiece / 8]), &eih->piecesMissing[curPiece / 8], 1);
    ...
    FLASH_Write(false, addr + offsetof(struct EepromImageHeader, validMarker), (uint8_t *)&eih->validMarker, sizeof(eih->validMarker));
...
void prvWriteNewHeader(struct EepromImageHeader *eih, uint32_t addr, uint32_t eeSize, uint64_t ver, uint32_t size)
{
    ...
    FLASH_Write(false, addr, (uint8_t *)eih, sizeof(struct EepromImageHeader));
}
...
int main(void)
{
    ...
    if (real_reason == 1)
    {
        ...
        sprintf(macStr, "(" MACFMT ")", MACCVT(mSelfMac));
        uiPrvFullscreenMsg("BOOTING", macStr, fwVerString());
    }
